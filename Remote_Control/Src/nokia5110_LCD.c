...
// вывод переменной строки
void LCD_var_str(uint8_t x, uint8_t y, uint16_t value, uint8_t nDigit)
{
	LCD_goXY(x, y);
	switch(nDigit)
	{
		case 5: LCD_putChar((value/10000)+'0');   // Если нужно перевести цифру 
		case 4: LCD_putChar((value/1000)+'0');   // Если нужно перевести цифру 
		case 3: LCD_putChar(((value/100)%10)+'0');//в символ, то следует 
		case 2: LCD_putChar(((value/10)%10)+'0');//добавить к цифре 
		case 1: LCD_putChar((value%10)+'0');     //символ '0' (или его код 48)
	}
}
